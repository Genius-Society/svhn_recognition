...
class ExampleReader(object):
    ...
    def _get_attrs(digit_struct_mat_file, index):
        ...
        for key in ["label", "left", "top", "width", "height"]:
            attr = f[item][key]
            attrs[key] = (
                [f[attr[i].item()][0][0] for i in range(len(attr))]
                if len(attr) > 1
                else [attr[0][0]]
            )
    ...
    def read_and_convert(self, digit_struct_mat_file):
        ...
        attrs_left, attrs_top, attrs_width, attrs_height = map(
...
def convert_to_lmdb(
    path_to_dataset_dir_and_digit_struct_mat_file_tuples,
    path_to_lmdb_dirs,
    ...
    for (
    ...
    ) in path_to_dataset_dir_and_digit_struct_mat_file_tuples:
        ...
        with h5py.File(path_to_digit_struct_mat_file, "r") as digit_struct_mat_file:
            ...
            for _ in tqdm(
                range(total_files),
                desc="%d files found in %s" % (total_files, path_to_dataset_dir),
            ):
